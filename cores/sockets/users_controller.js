...
SocketServer.on('connection', function(socket) {
	socket.lon('users.getMe', function(e, message, next) {
		if (socket.handshake.userId !== undefined) { 
			...
			Basbosa('Logger').info('the user Id that we need to check on it :'+ userId);
			var user = new User(), ObjectID = require('mongodb').ObjectID;
			
			user.findById(userId, function(error, user) {
				if(error) {
					Basbosa('Logger').info('error through check on userId :' + userId + 'in userAuthentication method');
				}
				if(!user) {
					Basbosa('Logger').debug('this userId :' + userId + ' not exist in the db', user);
					e.result.eventName = 'users.getMe_result';
					e.result.message.user = false;
				} else {
					Basbosa('Logger').debug('this userId :' + userId + ' is exist in the db', user);
